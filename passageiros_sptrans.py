...
def download_file(link, nome_arquivo, tentativas=3, timeout=8): # Valor padrão de 3 tentativas  # Timeout de 8 segundos
    ...
    for tentativa in range(tentativas):
        try:
            ...
            if response.status_code == 200:
                ...
                return True  # Retorna True se o arquivo foi baixado
            else:
    ...
    return False  # Retorna False se falhou em todas as tentativas
    

# Função para processar o arquivo
def processar_arquivo(nome_arquivo, mes_num, mes, ano):
...
for ano in anos:
    for mes_num, mes_nome in meses.items():
        ...
        if download_file(url, nome_arquivo):
            # Processar o arquivo baixado
            processar_arquivo(nome_arquivo, mes_num, mes_nome, ano)  # Chame a função aqui
